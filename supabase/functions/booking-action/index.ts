import { escapeHtml, getRequiredEnv, htmlResponse, sha256Hex } from '../_shared/http.ts';
...
Deno.serve(async (request) => {
  try {
    ...
    const reviewUrl = `${functionsBase}/booking-action?booking=${encodeURIComponent(booking.id)}&token=${encodeURIComponent(token)}`;
    ...
    if (action === 'decline') {
      ...
      try {
        await sendGuestDeclined(updated, reviewUrl);
      } catch (emailError) {
    ...
    const updated = data as BookingRow;
    try {
      await sendGuestConfirmed(updated, reviewUrl);
    } catch (emailError) {
    ...
    return htmlResponse(
      page(
        ...
        bookingId,
        token,
        'Booking confirmed. The dates remain blocked on the website and the guest can add the stay to their own calendar from the confirmation email.',
      ),
    );
  } catch (error) {
